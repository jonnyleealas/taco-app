...
controller.get("/", async (res: Response<ObjectLiteral[]>) => {
  const users = await useTypeORM(UserEntity).find();
  res.send(users);
});

controller.get("/:id", async (req: Request, res: Response<ObjectLiteral | ErrorObject>) => {
  const { id } = req.params;

  if (!id) {
    return res
      .status(400)
      .send({ message: "Required parameter \"id\" is missing!", statusCode: 400 });
  }
  ...
  if (!existingUser) {
    return res
      .status(404)
      .send({ message: `User with id: ${id} was not found.`, statusCode: 400  });
  }

  res.send(existingUser);
});

controller.put("/:id", async (req: Request, res: Response<ObjectLiteral | ErrorObject>) => {
  const { id } = req.params;
